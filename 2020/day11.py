    ...
    if is_part1:
        ...
        for r, c in product(range(row), range(col)):
            if grid[c, r] == 0:
                ...
                n += check(grid, cycle([c]),
                           range(r+1, row))     # right
                n += check(grid, reversed(range(c)), cycle([r]))          # up
                n += check(grid, range(c+1, col),
                           cycle([r]))          # down

                n += check(grid, reversed(range(c)),
                           reversed(range(r)))  # diag left up
                n += check(grid, range(c+1, col),
                           reversed(range(r)))  # diag left down
                n += check(grid, reversed(range(c)),
                           range(r+1, row))    # diag right up
                n += check(grid, range(c+1, col),
                           range(r+1, row))    # diag right down
