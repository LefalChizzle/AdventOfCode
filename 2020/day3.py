...
for slope in slopelist:
    ...
    for i in range(len(trees)):
        if i*slope[1] > len(trees): break
        trees[i*slope[1] % len(trees)][i*slope[0] % len(trees[i])] = "X" if trees[i*slope[1] % len(trees)][i*slope[0] % len(trees[i])] == "#" else "O"
        count += 1 if trees[i*slope[1] % len(trees)][i*slope[0] % len(trees[i])] == "X" else 0
